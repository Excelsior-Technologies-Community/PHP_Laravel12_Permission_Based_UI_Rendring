    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white shadow-sm border border-gray-200 rounded-2xl p-6 sm:p-8">
            ...
            <form method="POST" action="{{ route('products.update', $product) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="w-full text-sm border-gray-300 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="price" value="{{ old('price', $product->price) }}" step="0.01" min="0" required class="w-full text-sm border-gray-300 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                    </div>
                </div>
                ...
                @if($canViewCosts || $policy === 'disabled_lock')
                    <div class="pt-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-amber-50/40 p-4 rounded-xl border border-amber-200/60 {{ !$canEditCosts ? 'opacity-60 select-none' : '' }}">
                            <div>
                                ...
                                <input type="number" name="cost_price" 
                                       value="{{ $canViewCosts ? old('cost_price', $product->cost_price) : '' }}" 
                                       placeholder="{{ $canViewCosts ? 'e.g. 42000.00' : '•••••••• (Masked)' }}" 
                                       step="0.01" min="0" 
                                ...
                                <p class="text-[11px] text-gray-500 mt-1">
                                    @if($canViewCosts && $product->profit_margin)
                                        Current Margin: <strong class="text-emerald-700">{{ $product->profit_margin }}%</strong>
                                    @else
                                        Acquisition price used to compute gross profit margin.
                                    @endif
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="supplier_code" 
                                       value="{{ $canViewCosts ? old('supplier_code', $product->supplier_code) : '' }}" 
                                       placeholder="{{ $canViewCosts ? 'e.g. SUP-APPLE-DIRECT' : '•••••••• (Masked)' }}" 
                                       {{ !$canEditCosts ? 'disabled readonly' : '' }}
                                       class="w-full text-sm border-gray-300 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-indigo-500 {{ !$canEditCosts ? 'bg-gray-100 cursor-not-allowed' : '' }}">
